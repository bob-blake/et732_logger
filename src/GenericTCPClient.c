...
/*****************************************************************************
  Function:
	void TCPTimeToSend(void)

  Summary:
	Just sets a flag (a local variable) which alerts the TCP Client state
	machine that there is new data to send.	Added by Bob Blake, 12/31/12.
...
/*****************************************************************************
  ...
  Parameters:
	temp1: probe 1 temperature
	temp2: probe 2 temperature
	Both are pointers to strings, length is automatically parsed

  Returns:
  	None
  ***************************************************************************/
void GenericTCPClient(char* temp1, char* temp2, unsigned long timestamp)
//void GenericTCPClient(char* temp1, char* temp2)
{
	BYTE 				i;
	WORD				w;
	BYTE				vBuffer[9];
	...
	switch(GenericTCPExampleState)
	{
		...
		case SM_SOCKET_OBTAINED:
			...
			//sprintf(time,"%lu000",timestamp);	//Multiply by 1000 b/c javascript uses ms for UNIX epoch
			sprintf(time,"%lu",timestamp);	
		//	datalen = strlen(temp1) + strlen(temp2) + strlen(time) + 18;
			datalen = strlen(temp1) + strlen(temp2) + strlen(time) + 4;
			//datalen = strlen(temp1) + strlen(temp2) + 12;
			sprintf(datalen_s,"%u",datalen);
